...
class metData:
    ...
    # The 2* is done in Aginit.for lines 341-342
    gdko: float = field(default=2*0.15, metadata={'unit': 'm/s'})  # Vortext decay rate OGE  NEEDS AGCHK
    gdk: float = field(default=2*0.5588, metadata={'unit': 'm/s'})  # Vortext decay rate IGE   NEEDS AGCHK


    def changeMet(self, wind_speed):
        self.windspd = wind_speed

    def initializeMet(self, lfmaa):
        self.windspd = self.windspd
        self.zo = self.surfruff
        self.temptr = self.temp
        self.rhumtr = self.humidity

        item = min(int((self.windspd + 1.0) / 2.0) + 1, 5)
        ...
        self.usk = self.windspd / math.log((self.windht + self.zo) / self.zo) # The first usk does not include pstab
        self.qqmx = 0.845 * self.qstab * self.usk ** 2
        self.usk = self.windspd / (math.log((self.windht + self.zo) / self.zo) - self.pstab) # This is the second usk calc
        tem = self.winddr * self.tpi / 360.0
